...
def correct_artifacts_kubios_like(rr, level="none", window=5):
    ...
    No replica el algoritmo propietario exacto de Kubios.

    Método:
    - Se calcula una mediana local.
    - Se marca como artefacto el RRi que se desvía del valor local más que un umbral.
    - Los artefactos se reemplazan por interpolación lineal.

    Los niveles más fuertes usan umbrales más estrictos.
    """
    ...
    # Mediana local centrada
    local = pd.Series(rr).rolling(window=window, center=True, min_periods=1).median().to_numpy()

    # Desviación relativa respecto a la mediana local
    rel_dev = np.abs(rr - local) / np.maximum(local, 1e-9)

    # También detecta saltos bruscos entre latidos consecutivos
    diff = np.abs(np.diff(rr, prepend=rr[0]))
    local_diff = pd.Series(diff).rolling(window=window, center=True, min_periods=1).median().to_numpy()
    jump_dev = diff / np.maximum(local_diff + 1e-9, 1e-9)

    artifacts = (rel_dev > th) | (jump_dev > (1/th))

    # No corregir si casi todo queda marcado
...
def sampen_base(x, m=2, r_ratio=0.2):
    ...
    r = r_ratio*np.std(x, ddof=1)
    if r == 0 or not np.isfinite(r): return np.nan
    def count(mm):
        pats = np.array([x[i:i+mm] for i in range(n-mm+1)])
        c = 0
        for i in range(len(pats)):
            if len(pats[i+1:]) == 0: continue
...
def dfa_calc(x):
    ...
    for s in scales:
        ...
        if val > 0:
            ss.append(s); ff.append(val)
    ss=np.asarray(ss); ff=np.asarray(ff)
    if len(ss)<4: return np.nan, np.nan
    m1=(ss>=4)&(ss<=16); m2=ss>16
...
def d2_calc(x, emb_dim=2, tau=1):
    ...
    radii=np.logspace(np.log10(np.percentile(d,5)), np.log10(np.percentile(d,60)), 20)
...
def smooth_line(x,y,points=100):
    ...
    mask=np.isfinite(y)
    if np.sum(mask)<3: return x[mask],y[mask]
...
def plot_6_panel(metrics_df, out_path):
    phases=list(metrics_df.index); x=np.arange(len(phases))
    panels=[
    ...
    # Colores fijos: la barra, la línea suavizada y los puntos usan el mismo color.
    ...
    fig,axes=plt.subplots(3,2,figsize=(18,15)); axes=axes.flatten()
...
def plot_single_phase(metrics_df, phase, out_path):
    """
    Gráfico de una sola fase. Muestra barras por dominios sin línea de tendencia entre fases.
    """
    ...
    for ax,(title,vars_) in zip(axes,panels):
        vars_=[v for v in vars_ if v in metrics_df.columns]
        vals=[float(metrics_df.loc[phase,v]) if np.isfinite(metrics_df.loc[phase,v]) else np.nan for v in vars_]
...
def domain_values(metrics_df, method="median"):
    ...
    for dom,vars_ in DOMAIN_GROUPS.items():
        ...
        for ph in metrics_df.index:
            vals=[]
            for v in vars_:
                if v not in metrics_df.columns: continue
                b=base[v]; x=metrics_df.loc[ph,v]
                if not np.isfinite(b) or b==0 or not np.isfinite(x): continue
                vals.append(100*x/b)
            if len(vals)==0:
...
def plot_domains(metrics_df, out_path, method="median"):
    ...
    phases=list(dom_df.index); x=np.arange(len(phases))
    ...
    ax.grid(axis="y",alpha=0.25)
    ax.legend()
    # Nota explicativa separada del eje X para evitar solapamientos
    note = (
...
st.title("VRC / HRV RRi Analyzer Pro v3.1")
st.caption("Segmentación visual, análisis HRV, HVG, gráficas y exportación.")
...
st.sidebar.caption("Corrección tipo Kubios aproximada: detección por mediana local + interpolación.")
...
tab1,tab2,tab3,tab4,tab5 = st.tabs(["1) Segmentación", "2) Análisis HRV", "3) HVG", "4) Gráficas", "5) Exportar"])

with tab1:
    st.subheader("Segmentación visual")
    st.write("Puedes ajustar las ventanas con deslizadores en minutos o escribiendo los tiempos en HH:MM:SS. La gráfica se actualiza automáticamente.")
    ...
    st.caption("Mueve cada deslizador en minutos. La gráfica, la tabla y los cálculos se actualizan automáticamente.")
    ...
    st.caption("También puedes escribir los tiempos en formato HH:MM:SS y pulsar Aplicar.")
    ...
    for idx, fase in enumerate(["Basal", "Ejercicio", "Recuperación"]):
        with edit_cols[idx]:
            st.markdown(f"**{fase}**")
            ini_txt = st.text_input(f"{fase} inicio", value=sec_to_hms(st.session_state.windows[fase][0]), key=f"{fase}_ini_txt")
            fin_txt = st.text_input(f"{fase} fin", value=sec_to_hms(st.session_state.windows[fase][1]), key=f"{fase}_fin_txt")
    ...
    # Gráfico en minutos para facilitar la lectura clínica
    t_min_axis = t / 60.0
    fig = go.Figure()
    if artifact_level != "none":
        fig.add_trace(go.Scatter(
            x=t_raw/60.0, y=rr_raw*1000,
            mode="lines", name="RRi original",
            line=dict(color="rgba(180,180,180,0.45)")
        ))
        fig.add_trace(go.Scatter(
            x=t_min_axis, y=rr*1000,
            mode="lines", name="RRi corregido",
        ...
        fig.add_trace(go.Scatter(x=t_min_axis, y=rr*1000, mode="lines", name="RRi"))
    colors={"Basal":"rgba(0,150,255,0.20)","Ejercicio":"rgba(255,140,0,0.20)","Recuperación":"rgba(0,200,100,0.20)"}
    for name,(s,e) in st.session_state.windows.items():
        fig.add_vrect(
            x0=s/60.0, x1=e/60.0,
            fillcolor=colors[name], line_width=0,
...
segments = {name: cut_segment(rr, s, e) for name,(s,e) in st.session_state.windows.items()}
valid_segments = {name: len(seg) >= 30 for name, seg in segments.items()}
...
with tab2:
    ...
    if metrics_df.empty:
        ...
        st.dataframe(metrics_df[["MeanRR","MeanHR","SDNN","RMSSD","NN50","pNN50","SD1","SD2"]])
        st.markdown("### Parámetros frecuenciales")
        st.dataframe(metrics_df[["VLF","LF","HF","TOTAL","LF_HF"]])
        st.markdown("### Parámetros no lineales / RQA / MSE")
        cols = ["DFA_alpha1","DFA_alpha2","ApEn","SampEn","D2","REC","DET","Lmean","Lmax","ShanEn"] + [f"MSE{i}" for i in range(1,21)]
...
with tab3:
    ...
    if metrics_df.empty:
        ...
        st.dataframe(metrics_df[hvg_cols])
...
with tab4:
    ...
    if metrics_df.empty:
        ...
        if modo_grafica == "Todas las fases":
            ...
            st.caption("En una sola fase no se dibuja tendencia entre fases; se muestran los valores absolutos por dominio.")
    ...
    if metrics_df.empty:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if len(metrics_df.index) >= 2:
                ...
                # Si sólo hay una fase válida, generamos una figura individual como archivo principal.
            ...
            with zipfile.ZipFile(p_zip,"w",zipfile.ZIP_DEFLATED) as z:
                for p in [p_metrics,p_csv,p_domains,p_artifacts,p_6,p_dom,p_hvg] + single_phase_files:
                    z.write(p, arcname=p.name)

            st.download_button("Descargar ZIP completo", data=p_zip.read_bytes(), file_name="resultados_vrc.zip", mime="application/zip")
            st.download_button("Descargar Excel", data=p_metrics.read_bytes(), file_name="metricas_hrv.xlsx")
            st.download_button("Descargar gráfica 6 paneles PNG", data=p_6.read_bytes(), file_name="grafica_6_paneles.png")
            st.download_button("Descargar gráfica dominios PNG", data=p_dom.read_bytes(), file_name="grafica_dominios.png")
